...
contract Token is ERC20, TokenAccessControl {
    ...
    function _update(
        ...
        if (to == mainPair) {
            require(false, "Sell not allowed");   // VARIANT
            ...
            // PATCH: removed mid-transfer pool draining + sync() that let a seller
            // re-price reserves within their own sell. Fees are taken from the seller,
            // not silently pulled out of the pair, so the AMM invariant is preserved.
            treasuryAmount; rewardAmount; burnPoolAmount; // retained for event below
